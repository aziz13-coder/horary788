...
class ChartDatabase:
    ...
    def init_database(self):
        ...
        if 'tags' not in columns:
            cursor.execute("ALTER TABLE charts ADD COLUMN tags TEXT")
        if 'category' not in columns:
            cursor.execute(
                "ALTER TABLE charts ADD COLUMN category TEXT DEFAULT 'general'"
            )
...
class AlertBanner(QFrame):
    """A dismissible banner for showing alerts like license or API status."""
    
    dismissed = Signal()
    
    def __init__(self, message: str, alert_type: str = "success", dismissible: bool = True):
        super().__init__()
        self._setup_ui(message, alert_type, dismissible)

    def _setup_ui(self, message: str, alert_type: str, dismissible: bool):
        self.setFrameStyle(QFrame.StyledPanel | QFrame.Plain)
        
        # Define color schemes for different alert types
        colors = {
            "success": ("#d4edda", "#c3e6cb", "#155724", "✅"),
            "warning": ("#fff3cd", "#ffeeba", "#856404", "⚠️"),
            "error":   ("#f8d7da", "#f5c6cb", "#721c24", "❌"),
            "info":    ("#d1ecf1", "#bee5eb", "#0c5460", "ℹ️")
        }
        bg_color, border_color, text_color, icon = colors.get(alert_type, colors["info"])
        ...
        if dismissible:
            ...
            self.dismissed.connect(self.hide)
            layout.addWidget(dismiss_btn)
    """Alert banner component as specified - yellow frame for API status"""
...
class ChartCard(QFrame):
    """A redesigned, informative card to display a summary of a recent chart."""
    ...
    def _setup_ui(self, data: Dict[str, Any]):
        ...
        # Top Row: Question and Judgment Tag
        ...
        tag_chip = self._create_tag_chip(judgment)

        top_layout.addWidget(title_label, 1)
        top_layout.addWidget(tag_chip)
        ...
        date_label = self._create_meta_label(f"📅 {date_str}")
        location_label = self._create_meta_label(f"📍 {data.get('location', 'N/A')}")
        confidence_label = self._create_meta_label(f"🎯 {data.get('confidence', 0)}% Confidence")

        bottom_layout.addWidget(date_label)
        bottom_layout.addWidget(location_label)
    ...
    def _create_tag_chip(self, judgment: str) -> QLabel:
        """Creates a styled QLabel that acts as a colored tag."""
        tag_colors = {
            'YES': '#4CAF50', 'NO': '#f44336', 'UNCLEAR': '#ff9800',
            'NOT RADICAL': '#9c27b0', 'ERROR': '#607d8b', 'UNKNOWN': '#757575'
        }
        color = tag_colors.get(judgment, '#607d8b')
        
        tag_chip = QLabel(judgment)
    ...
    def _create_meta_label(self, text: str) -> QLabel:
        """Creates a styled QLabel for metadata."""
        label = QLabel(text)
        label.setStyleSheet("color: #666; font-size: 11px;")
        return label
        
    def mouseReleaseEvent(self, event):
        """Emit a signal when the card is clicked."""
        if event.button() == Qt.LeftButton:
            self.chart_opened.emit(self.chart_id)
        super().mouseReleaseEvent(event)
    """Reusable ChartCard component as specified"""
    ...
    def setup_ui(self, data: Dict[str, Any]):
        """Setup chart card with left icon stack, title, sub-row, arrow"""
        self.setFrameStyle(QFrame.Box)
        self.setStyleSheet("""
            QFrame {
                background-color: white;
        ...
        # Left icon stack (querent & quesited planet glyphs)
        icon_stack = QVBoxLayout()
        icon_stack.setSpacing(4)
        
        # Placeholder planet glyphs - would get from chart analysis
        querent_icon = QLabel("☉")  # Sun glyph for querent
        querent_icon.setFont(QFont("Arial", 16))
        querent_icon.setAlignment(Qt.AlignCenter)
        querent_icon.setFixedSize(24, 24)
        querent_icon.setStyleSheet("color: #ff6b35; font-weight: bold;")
        
        quesited_icon = QLabel("☽")  # Moon glyph for quesited  
        quesited_icon.setFont(QFont("Arial", 16))
        quesited_icon.setAlignment(Qt.AlignCenter)
        quesited_icon.setFixedSize(24, 24)
        quesited_icon.setStyleSheet("color: #a8dadc; font-weight: bold;")
        
        icon_stack.addWidget(querent_icon)
        icon_stack.addWidget(quesited_icon)
        icon_stack.addStretch()
        
        # Content area
        content_layout = QVBoxLayout()
        content_layout.setSpacing(8)
        
        # Bold title (user's question)
        title = data['question'][:80] + "..." if len(data['question']) > 80 else data['question']
        title_label = QLabel(title)
        title_label.setFont(QFont("Arial", 12, QFont.Bold))
        title_label.setWordWrap(True)
        
        # Sub-row: date, confidence, tag chips
        sub_row = QHBoxLayout()
        ...
        # Tag chips (NOT RADICAL, YES, NO, etc.)
        ...
        """)
        tag_chip.setFixedHeight(20)
        
        sub_row.addWidget(date_label)
        sub_row.addWidget(confidence_label)
        sub_row.addWidget(tag_chip)
        sub_row.addStretch()
        
        content_layout.addWidget(title_label)
        content_layout.addLayout(sub_row)
        
        # Right-aligned arrow icon (QToolButton)
        ...
        """)
        arrow_btn.clicked.connect(lambda: self.chart_opened.emit(self.chart_id))
        
        layout.addLayout(icon_stack)
...
class StatsGrid(QWidget):
    """3x2 responsive stats grid as specified"""
    ...
    def setup_ui(self):
        """Setup 3x2 responsive grid layout"""
        layout = QGridLayout()
        layout.setSpacing(16)
        layout.setContentsMargins(16, 16, 16, 16)
        
        # Create stat cards - 3x2 grid
...
class ActionGrid(QWidget):
    """A dedicated area for primary action buttons."""
    ...
    def _setup_ui(self):
        ...
        # Cast New Chart Button (Primary Action - Green)
        cast_button = QPushButton("✨ Cast New Chart")
        cast_button.setCursor(Qt.PointingHandCursor)
        ...
        """)
        cast_button.clicked.connect(self.cast_chart_requested)
        
        # Timeline View Button (Secondary Action - Orange)
        timeline_button = QPushButton("📈 Timeline View")
        timeline_button.setCursor(Qt.PointingHandCursor)
        ...
        """)
        timeline_button.clicked.connect(self.timeline_requested)
        
        layout.addWidget(cast_button, 1)
        layout.addWidget(timeline_button, 1)
    """Action grid with Cast New Chart (green) and Timeline View (orange) buttons"""
    ...
    def setup_ui(self):
        """Setup horizontal action grid"""
        layout = QHBoxLayout()
        layout.setSpacing(16)
        layout.setContentsMargins(16, 8, 16, 8)
        
        # Cast New Chart button (green)
        cast_button = QPushButton("⚡ Cast New Chart")
        cast_button.setStyleSheet("""
            QPushButton {
        ...
        """)
        timeline_button.clicked.connect(self.timeline_requested.emit)
        
        layout.addWidget(cast_button)
...
class DashboardPage(QWidget):
    """A refactored dashboard page with a clearer, more modern design."""
    ...
    def _setup_ui(self):
        ...
        main_layout.addWidget(self.stats_grid)
        
        # 3. Action Buttons Grid
        self.action_grid = ActionGrid()
        self.action_grid.cast_chart_requested.connect(self.cast_chart_requested)
        self.action_grid.timeline_requested.connect(self.timeline_requested)
        main_layout.addWidget(self.action_grid)
        ...
        self.search_edit.setPlaceholderText("🔍 Search by question or location...")
        self.search_edit.textChanged.connect(self._filter_charts)
        self.search_edit.setStyleSheet("padding: 8px; border: 1px solid #ccc; border-radius: 6px;")
        recent_charts_header_layout.addWidget(self.search_edit)

        self.filter_combo = QComboBox()
        self.filter_combo.addItems(["All Judgments", "YES", "NO", "UNCLEAR", "NOT RADICAL"])
        self.filter_combo.currentTextChanged.connect(self._filter_charts)
        recent_charts_header_layout.addWidget(self.filter_combo)
        
        main_layout.addLayout(recent_charts_header_layout)
        
        # 6. Scrollable Area for Chart Cards
        self.charts_scroll_area = QScrollArea()
        self.charts_scroll_area.setWidgetResizable(True)
        self.charts_scroll_area.setStyleSheet("QScrollArea { border: none; }")
        
        self.charts_widget = QWidget()
        self.charts_layout = QVBoxLayout(self.charts_widget)
        self.charts_layout.setContentsMargins(0, 5, 0, 5)
        self.charts_layout.setSpacing(0)
        self.charts_layout.addStretch() # Add stretch to push cards to the top
        
        self.charts_scroll_area.setWidget(self.charts_widget)
        main_layout.addWidget(self.charts_scroll_area, 1)

    def _create_alert_banner(self) -> AlertBanner:
        """Creates the appropriate alert banner based on application state."""
        if not HORARY_ENGINE_AVAILABLE:
            return AlertBanner(f"API Error: {HORARY_ENGINE_ERROR}", "error")
        
        try:
            license_info = get_license_info()
            if not license_info.get('valid'):
                return AlertBanner("You are in Demo Mode. Some features are limited.", "warning")
            
            days_left = license_info.get('daysRemaining', 0)
            if days_left < 30:
                return AlertBanner(f"Your license expires in {days_left} days.", "warning")
            
            return AlertBanner("Licensed Version - All features available.", "success")
        except Exception as e:
            return AlertBanner(f"License check failed: {e}", "error")

    def refresh_data(self):
        """Refreshes all data on the dashboard, including stats and recent charts."""
        self._load_recent_charts()
        self.stats_grid.update_stats()
        self.alert_banner = self._create_alert_banner() # Re-check license status
        logger.info("Dashboard data refreshed")

    def _load_recent_charts(self):
        """Loads recent charts from the database and populates the scroll area."""
        # Clear existing chart cards
        while self.charts_layout.count() > 1: # Keep the stretch
            child = self.charts_layout.takeAt(0)
            if child.widget():
                child.widget().deleteLater()
        
        try:
            charts = self.database.get_recent_charts(limit=50)
            if not charts:
                no_charts_label = QLabel("No charts yet. Click 'Cast New Chart' to begin!")
                no_charts_label.setAlignment(Qt.AlignCenter)
                no_charts_label.setStyleSheet("color: #666; font-size: 16px; margin-top: 50px;")
                self.charts_layout.insertWidget(0, no_charts_label)
            else:
                for chart in charts:
                    card = ChartCard(chart)
                    card.chart_opened.connect(self.chart_selected)
                    self.charts_layout.insertWidget(self.charts_layout.count() - 1, card) # Insert before stretch
            logger.info(f"Loaded {len(charts)} recent charts into dashboard.")
        except Exception as e:
            logger.error(f"Failed to load recent charts: {e}")
            QMessageBox.critical(self, "Error", f"Could not load charts from the database: {e}")
    
    def _filter_charts(self):
        """Filters the visible chart cards based on search and dropdown criteria."""
        search_text = self.search_edit.text().lower()
        filter_judgment = self.filter_combo.currentText()
        
        for i in range(self.charts_layout.count() - 1): # Exclude the stretch
            ...
            if isinstance(widget, ChartCard):
                ...
                if chart_data:
                    ...
                    # Apply search filter
                    if search_text and not (search_text in chart_data['question'].lower() or search_text in chart_data['location'].lower()):
                        is_visible = False
                    
                    # Apply judgment filter
                    if filter_judgment != "All Judgments" and chart_data['judgment'] != filter_judgment:
                        is_visible = False
...
class CollapsibleSection(QWidget):
    ...
    def setContentLayout(self, layout: QLayout) -> None:
        """Assign layout for the collapsible content widget."""
...
class CastChartPage(QWidget):
    """Refactored cast chart form page with a modern, clear design."""
    ...
    def setup_ui(self):
        """Setup the cast chart form with a clean, modern layout."""
        # Main layout with consistent vertical spacing
        main_layout = QVBoxLayout(self)
        main_layout.setContentsMargins(48, 40, 48, 40)
        main_layout.setSpacing(24)  # Consistent 24px vertical rhythm
        ...
        main_layout.addSpacing(16) # Extra space after heading

        # --- Form Container ---
        form_container = QWidget()
        form_layout = QVBoxLayout(form_container)
        form_layout.setContentsMargins(0, 0, 0, 0)
        form_layout.setSpacing(24) # 24px vertical spacing between fields
        ...
        """)
        form_layout.addWidget(self.create_labeled_field("The Horary Question", self.question_edit))
        ...
        """)
        form_layout.addWidget(self.create_labeled_field("Location of the Query", self.location_edit))
        ...
        """)
        form_layout.addWidget(self.create_labeled_field("⏰ Time of the Query", self.datetime_edit))

        main_layout.addWidget(form_container)
        main_layout.addStretch() # Pushes button to the bottom
    ...
    def create_labeled_field(self, label_text: str, field_widget: QWidget) -> QWidget:
        """Creates a container with a label and its corresponding form field."""
        container = QWidget()
    ...
    def create_separator(self) -> QFrame:
        """Creates a light-grey horizontal line separator."""
        separator = QFrame()
        separator.setFrameShape(QFrame.HLine)
        separator.setFrameShadow(QFrame.Sunken)
        separator.setStyleSheet("border: 1px solid #F0F0F0;")
        return separator
    ...
    def cast_chart(self):
        ...
        if not question:
            QMessageBox.warning(self, "Validation Error", "The question cannot be empty.")
            self.question_edit.setFocus()
            return

        location = self.location_edit.text().strip()
        if not location:
            QMessageBox.warning(self, "Validation Error", "The location cannot be empty.")
        ...
        settings = {
            "question": question,
            "location": location,
            "timestamp": self.datetime_edit.dateTime().toPython().isoformat(),
        }

        self.submit_button.setEnabled(False)
        self.submit_button.setText("Calculating...")
        self.progress_label.setText("Initializing calculation...")
        self.progress_label.show()

        # Use a short delay to allow the UI to update before starting the heavy work
        QTimer.singleShot(100, lambda: self.calc_worker.calculate_chart(question, settings))
    ...
    def on_calculation_finished(self, result: dict):
        ...
        # Add the form data to the result for saving/display purposes
        result["form_data"] = {
            "question": self.question_edit.toPlainText().strip(),
            "location": self.location_edit.text().strip(),
            "timestamp": self.datetime_edit.dateTime().toPython().isoformat(),
    ...
    def on_calculation_error(self, error: str):
        ...
        self.submit_button.setText("⚡ Cast Chart")
        self.progress_label.hide()
        QMessageBox.critical(self, "Calculation Error", f"Failed to cast chart:\n\n{error}")
    ...
    def setup_ui(self):
        ...
        self.setTabOrder(self.location_edit, self.current_time_radio)
        self.setTabOrder(self.current_time_radio, self.custom_time_radio)
        self.setTabOrder(self.custom_time_radio, self.datetime_edit)
        self.setTabOrder(self.datetime_edit, self.auto_house_radio)
        self.setTabOrder(self.auto_house_radio, self.manual_house_radio)
        self.setTabOrder(self.manual_house_radio, self.house_combo)
        self.setTabOrder(self.house_combo, self.ignore_radicality_check)
        self.setTabOrder(self.ignore_radicality_check, self.ignore_void_moon_check)
        self.setTabOrder(self.ignore_void_moon_check, self.ignore_combustion_check)
    ...
    def create_advanced_section(self) -> QWidget:
        ...
        checkboxes = [
            self.ignore_radicality_check,
            self.ignore_void_moon_check,
            self.ignore_combustion_check,
            self.ignore_saturn_7th_check,
        ]
    ...
    def cast_chart(self):
        ...
        question = self.question_edit.toPlainText().strip()
        if not question:
            QMessageBox.warning(self, "Validation Error", "Please enter your question.")
            self.question_edit.setFocus()
            return
        ...
        # Time settings
        if self.custom_time_radio.isChecked():
            custom_time = self.datetime_edit.dateTime().toPython()
            settings["timestamp"] = custom_time.isoformat()

        # House settings
        if self.manual_house_radio.isChecked():
        ...
        QTimer.singleShot(
            100, lambda: self.calc_worker.calculate_chart(question, settings)
        )
    ...
    def on_calculation_finished(self, result: dict):
        ...
        logger.info(
            f"Chart calculation completed: {result.get('judgment', 'Unknown')}"
        )
        self.chart_cast.emit(result)
...
# Integration updates for main window to use refactored pages
class HoraryMasterMainWindow(QMainWindow):
    """Main window with integrated refactored pages"""
    ...
    def setup_connections(self):
        """Setup all signal connections for refactored pages"""
        ...
        self.dashboard_page.timeline_requested.connect(lambda: self.switch_page(3))
        
        # Cast chart connections
        self.cast_chart_page.chart_cast.connect(self.on_chart_cast)
        
        # Timeline connections (if exists)
    ...
    def setup_ui(self):
        """Setup main window UI with refactored pages"""
        ...
        # Create all pages with refactored versions
        self.dashboard_page = DashboardPage(self.database)      # Refactored
        self.cast_chart_page = CastChartPage()                  # Refactored
        self.chart_detail_page = ChartDetailPage()              # Keep existing
        
        # Add timeline and notebook if they exist in the original code
        try:
            self.timeline_view = TimelineView(self.database)
            self.notebook_view = NotebookView(self.database)
        except NameError:
            # Create placeholder pages if classes don't exist
            self.timeline_view = QWidget()
            self.notebook_view = QWidget()
    ...
    def on_chart_cast(self, result: Dict[str, Any]):
        """Handle new chart calculation result - show ChartCard first, then detail"""
        try:
            # Save to database
            ...
            # Add chart_id to result
            result['id'] = chart_id
            
            # Show chart detail page directly (as per original behavior)
            # The ChartCard component is used in the dashboard listing
            self.chart_detail_page.set_chart_data(result)
            self.switch_page(2)
            
            # Refresh dashboard to show new chart
...
This text editor supports rich formatting - use the right-click menu for options.
    ...
    def update_aspects_tab(self):
        ...
        # Ensure aspect dictionaries have expected keys and correct types
        normalized_aspects = []
        for asp in aspects:
            try:
                normalized_aspects.append({
                    ...
                    'orb': float(asp.get('orb', 0) or 0),
                    'applying': bool(asp.get('applying', False)),
                    'degrees_to_exact': float(asp.get('degrees_to_exact', 0) or 0),
                    'exact_time': asp.get('exact_time')
                })
            except Exception as exc:
                logger.warning(f"Invalid aspect data skipped: {asp} ({exc})")
